...
if (isset($_GET['hapus'])) {
    ...
    $db->prepare("DELETE FROM tb_paket WHERE id = ?")->execute([$_GET['hapus']]);
    header('Location: paket.php?msg=' . urlencode('Paket berhasil dihapus')); exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_POST['id']) {
        ...
        header('Location: paket.php?msg=' . urlencode('Paket berhasil diupdate')); exit;
    } else {
        $db->prepare("INSERT INTO tb_paket (nama_paket, jenis, harga, id_outlet) VALUES (?,?,?,?)")
           ->execute([$nama_paket, $jenis, $harga, $id_outlet]);
        header('Location: paket.php?msg=' . urlencode('Paket berhasil ditambahkan')); exit;
    }
}
...
<div class="main-content">
    <div class="topbar">
        ...
        <h6><i class="bi bi-box-seam me-2"></i>Paket Laundry</h6>
    </div>
